...
export default function ShippingAddressScreen(props){
    const userSignin = useSelector((state) => state.userSignin); // If the user sign out in the shippingAddressScreen, app needs to redirect user to the SigninScreen
    const {userInfo} = userSignin;
    const cart = useSelector((state) => state.cart); // If the user already placed an order before then shippingAddress will be filled automatically based on the previous one
    ...
    const submitHandler = (e) => {
        ...
        props.history.push("/placeorder"); // payment and add step 3 to checkoutsteps below
    } 
    return (
        <div>
            ...
            <form className="form" onSubmit={submitHandler}>
                ...
                <div>
                    <label>
                        <button className="block-green" type="submit">Ödemeye geç</button>
                    </label>
                </div>
            </form>
        </div>
